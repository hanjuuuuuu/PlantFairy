...
class Leaf {
  constructor(parent, length) {
    ...
    this.true_color = this.colors[Math.floor(Math.random() * this.colors.length)];
...
class Plant {
  constructor(x, y, l) {
    ...
    this.links = [];
    this.length = l;
    this.resolution = Math.floor(Util.map(this.length, 0, H, 3, 8)) + Math.round(Util.random(0, 2));

    this.div = this.length / this.resolution;
    this.min_thickness = 10;
    this.max_thickness = 32;
    this.thickness = Util.map(this.length, H, 0, this.min_thickness, this.max_thickness) * Util.random(0.8, 1.2);
    this.z_value = 0;
  ...
  init() {
    ...
    for (let i = 1; i < this.resolution; i++) {
      let leaf_size = Util.map(this.thickness, this.min_thickness, this.max_thickness, 10, 60);
  ...
  update() {
    ...
    for (let i = 0; i < 4; i++) {
      this.links.forEach((link) => {
        ...
        let offset = new Vector((link.p1.position.x - link.p0.position.x) * percent, (link.p1.position.y - link.p0.position.y) * percent);
...
let wind = {
  ...
  update: function () {
    ...
    if (time < this.duration) {
      this.value.x = Tween.linear(time, this.start, this.goal - this.start, this.duration);
    } else {
      setTimeout(() => {
        this.reset();
